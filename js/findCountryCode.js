...
/* 
    ...
    const countryName = countries.filter(findCntry);

    function findCntry(cn) {
        if(countryName == cn) {
...
function findCountry(countryName) {
    ...
    countries.forEach(cntry => {
        ...
        cntryName = cntry.name;
        if (cntry.name.toLowerCase() == countryName) {
            find = true;
            console.log(`The country code of ${cntryName} is ${cntryCode}`);
        }

    });

    if (!find) {
        console.log(`The country name ${cntryName} is invalid. Please enter correct country.`);
    }
}

// findCountry("Nepal");
// findCountry("zimbabwe");
// findCountry("zimbabwase");
